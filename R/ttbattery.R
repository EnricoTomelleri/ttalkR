...
p + geom_point(aes(colour = id_col), size = 0.2) +
  ...
  geom_line(data=df1, aes(HR_Timestamp_4B, Bat_mV), color = "black") +
  geom_segment(aes(x = min(HR_Timestamp_4D, na.rm=T), y = 3500, xend = max(HR_Timestamp_4D, na.rm=T), yend = 3500), color = "red") +
  labs(x = "Timestamp") +
  #labs(title = site) +
