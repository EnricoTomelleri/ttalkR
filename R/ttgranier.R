ttgranier <- function(mydata_4D, plot_label){
  ...
  Tref_0C <- 127.6-0.006045*mydata_4D$Tref_0 + 1.26E-7*mydata_4D$Tref_0^2 -1.15E-12*mydata_4D$Tref_0^3
  ...
  ############################
  Tref_1C <- 127.6-0.006045*mydata_4D$Tref_1 + 1.26E-7*mydata_4D$Tref_1^2 -1.15E-12*mydata_4D$Tref_1^3
  ...
  ############################
  Theat_0C <- 127.6-0.006045*mydata_4D$Theat_0 + 1.26E-7*mydata_4D$Theat_0^2 -1.15E-12*mydata_4D$Theat_0^3
  ...
  ############################
  Theat_1C <- 127.6-0.006045*mydata_4D$Theat_1 + 1.26E-7*mydata_4D$Theat_1^2 -1.15E-12*mydata_4D$Theat_1^3
  ...
  dTmax <- (dTon - dToff) #max(Theat_1C-Theat_0C, na.rm=T)#max(Theat_1C-Theat_0C, na.rm=T)

  ID <- unique(mydata_4D$IT_ID)
  for (j in 1:(length(ID))){
    #dTmax[mydata_4D$IT_ID == ID[j]] <- (dTon[mydata_4D$IT_ID == ID[j]]  - dToff[mydata_4D$IT_ID == ID[j]] )
    df <- data.frame(dTmax[mydata_4D$IT_ID == ID[j]], mydata_4D$SDate[mydata_4D$IT_ID == ID[j]])
    colnames(df) <- c("dTmax", "Date")
    if(length(na.omit(df$dTmax))<11){next()}
  ...
  Fd <- 118.99*((mydata_4D$daily_Tmax-(dTon-dToff))/(dTon-dToff))^1.231 #m^3/s
  Fd[Fd>1000] <- NA
  ...
  for (j in 1:(length(ID))){
    ...
    ts_filt <- baytrends::fillMissing(ts, span = 12, Dates = NULL, max.fill = 12)#gapfillSSA(series = ts, plot.results = FALSE, open.plot = FALSE)
    #ts_filt <- ts_filt[[1]]
